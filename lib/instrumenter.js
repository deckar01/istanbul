    ...
    Instrumenter.prototype = {
        ...
        filterHints: function (comments) {
            ...
            for (i = 0; i < comments.length; i += 1) {
                ...
                if (comment && comment.value && comment.range && isArray(comment.range)) {
                    ...
                    if (groups) {
                        ret.push({ action: groups[1], type: groups[2], start: comment.range[0], end: comment.range[1] });
                    }
                }
            }
            return ret;
        ...
        maybeSkipNode: function (node, type) {
            var alreadyIgnoring = !!this.currentState.ignoring,
                ...
                ignoreThis = !alreadyIgnoring && hint && hint.action === 'ignore' && hint.type === type;
        ...
        ifBranchInjector: function (node, walker) {
            var alreadyIgnoring = !!this.currentState.ignoring,
                hint = this.currentState.currentHint,
                ignoreThen = !alreadyIgnoring && hint && hint.action === 'ignore' && hint.type === 'if',
                ignoreElse = !alreadyIgnoring && hint && hint.action === 'ignore' && hint.type === 'else',
        ...
        maybeAddSkip: function (branchLocation) {
            return function (node) {
                var alreadyIgnoring = !!this.currentState.ignoring,
                    hint = this.currentState.currentHint,
                    ignoreThis = !alreadyIgnoring && hint && hint.action === 'ignore' && hint.type === 'next';
                if (ignoreThis) {
                    this.startIgnore();
                    node.postprocessor = this.endIgnore;
                }
